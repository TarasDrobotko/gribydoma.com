...
jQuery(document).ready(function($) {
    ...
    /* $('#callbackForm').simpleSendForm({
        ...
        autoClose: true,
        autoCloseDelay: 3000,
        mailUrl: "/ajax-form/form-submit/submit.php",
        captcha: false,
    });  */

      //Data for hidden fields of the form
      $('.button').click(function(event){
        var button = event.currentTarget;
        var div = button.parentElement.parentElement;

        var price = div.previousElementSibling.innerHTML;
        var price_split = price.split('.');
        var price = price_split[0];
        if (div.previousElementSibling.lastElementChild.classList.contains('price-new')) {
            var price = div.previousElementSibling.lastElementChild.innerHTML;
            var price_split = price.split('.');
            var price = price_split[0];

        }

        var price_elem = div.previousElementSibling;

        var product_name = price_elem.previousElementSibling.firstElementChild.innerHTML;

        var product_name_elem = price_elem.previousElementSibling.firstElementChild;
        var product_href = product_name_elem.href;
       
        var product_href_split = product_href.split('=');
        var product_id = product_href_split[2]; 
